...
ASTWalker.prototype = {
  ...
  "translateArgs": function(args) {
    var flag = this.argsTranslate;
    this.argsTranslate = true;
    var result = this.translateAST(args, ",");
    this.argsTranslate = flag;
    return result;
  },

  // TODO: We could use let instead of wrapping in a function
  "translateLoop": function(current, fun) {
    var condition = this.translateNode(current[1]);
    var exprs = ["FUNCALL", ["FUNCTION", [], current[2]], []];
    var flag = this.loopTranslate;

    this.loopTranslate = true;
    exprs = this.translateNode(exprs);
    this.loopTranslate = flag;

    return fun(condition, exprs);
  },
  ...
  "REPEAT": function(current) {
    return this.translateLoop(current, function(condition, exprs){
      return "do{\r\nif(" + exprs + " === true) break;} while(__lua.isFalse(" + condition + "))";
    });
  },
  ...
  "BREAK": function(current) {
    return this.loopTranslate ? "return true" : "break";
  },
