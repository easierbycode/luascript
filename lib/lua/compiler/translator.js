...
ASTWalker.prototype = {
  ...
  "NUMBER": function(current) {
    return this.stripZeros(current[1]);
  },

  "HEXNUMBER": function(current) {
    return current[1];
  },

  "STRING": function(current) {
    var translator = this;
    return current[1].replace(
      /(^|[^\\])\\(\d{1,3})/g,
      function(match, p1, p2){
        return p1 + "\\" + parseInt(translator.stripZeros(p2)).toString(8);
      }
    );
  }
}
